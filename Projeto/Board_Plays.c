...
int word_input(char board[16][16], char input_column, int input_line, char orientation, char word[1][16],int *global_score, int *play_nr, int exit, int mode, int MAX_SIZE)
{
    int i=0, score, letter_reuse=0, new_letter=0, valid_play=0, input_column_int=0,wordlen=0;
    ...
    input_column_int=input_column;

    //Verificação das condições para a primeira jogada
    }
    if(*play_nr==0){
        if(orientation=='H' && input_line==((MAX_SIZE-1)/2)+1 && ((input_column+wordlen-1)>=((MAX_SIZE+1)/2))){
            if((wordlen+(input_column))>=MAX_SIZE){
                if(mode==1){
            ...
            *global_score=*global_score+score;
            ...
            for(int column_aux=input_column; column_aux<wordlen+input_column; column_aux++){
    ...
    if(*play_nr!=0){
        if(orientation=='H'){
            for(int column_aux=input_column;column_aux<(wordlen+(input_column));column_aux++){
                ...
                if(board[input_line][column_aux]==word[0][column_aux-input_column]|| board[input_line][column_aux]=='$' || board[input_line][column_aux]=='2' || board[input_line][column_aux]=='.' || board[input_line][column_aux]=='3'){
                    if(((wordlen+(input_column-1))<=MAX_SIZE)){ // Verificar se a palavra cabe no board
                    }
                    else{
                        if(mode==1){
                            printf("The word doesn't fit the board\n");
        ...
        if(orientation=='V'){
            for(int line_aux=input_line;line_aux<(wordlen+(input_line));line_aux++){
                if(board[line_aux][input_column]==word[0][line_aux-input_line]){
                }
                else if(board[line_aux][input_column]=='$' || board[line_aux][input_column]=='2' || board[line_aux][input_column]=='.' || board[line_aux][input_column]=='3'){
                    new_letter=1;
                }
                if(board[line_aux][input_column]==word[0][line_aux-input_line] || board[line_aux][input_column]=='$' || board[line_aux][input_column_int]=='2' || board[line_aux][input_column_int]=='.' || board[line_aux][input_column]=='3'){
                    if((((strlen(word))+(input_line-1))<=MAX_SIZE)){ // A palavra cabe no board
                    }
                    else{
                        if (mode==1){
                            printf("The word doesn't fit the board\n");
            ...
            if(new_letter==1 && letter_reuse==1){
                ...
                *global_score=*global_score+score;
                ...
                for(int line_aux=input_line;line_aux<(wordlen+(input_line));line_aux++){
                        board[line_aux][input_column]=word[0][line_aux-input_line];
                        i=i+1;
                }
            }
            else{
    ...
    for(int k=0; k<=(((4+(3*MAX_SIZE))-wordlen)/2)-2; k++){
        printf(" ");
    }
    printf("%c%d%c %s %d pontos\n", input_column+64, input_line, orientation, word, score);
...
int score_counter(char board[16][16], char word[0][16], int input_line, char input_column, char orientation, int MAX_SIZE)
{
    ...
    for(i=0; i<strlen(word[0]); i++){ //ler a palavra toda
                if (orientation=='H'){ //destinção para saber se a jogada foi feita na horizontal ou na vertical
                        if(board[input_line][input_column+i]=='$'){
                        ...
                        if(board[input_line][input_column+i]=='3'){ //pontuação referente a cada letra caso calhe numa casa com o character '3'
                        ...
                        if(board[input_line][input_column+i]=='2'){//pontuação referente a cada letra caso calhe numa casa com o character '2'
                            if(word[0][i]== 'A' || word[0][i]== 'E' || word[0][i]== 'I' || word[0][i]== 'L' || word[0][i]== 'N' || word[0][i]== 'O' || word[0][i]== 'R' || word[0][i]== 'S' || word[0][i]== 'T' || word[0][i]== 'U' || word[0][i]== 'a' || word[0][i]== 'e' || word[0][i]== 'i' || word[0][i]== 'l' || word[0][i]== 'n' || word[0][i]== 'o' || word[0][i]== 'r' || word[0][i]== 's' || word[0][i]== 't' || word[0][i]== 'u'){
                                score=score + (2*1);
                                break;
                            }
                        ...
                        if(board[input_line][input_column+i]=='.' || board[input_line][input_column+i]=='$' || board[input_line][input_column+i]== word[0][i]){//pontuação referente a cada letra caso calhe numa casa com o character '.' ou '$' ou numa casa que ja contenha uma letra, visto que no caso do '$' o seu multiplicador ja foi contabilizado e no caso de uma casa repetida os multiplicadores do tabuleiro ja não se aplicam
                            if(word[0][i]== 'A' || word[0][i]== 'E' || word[0][i]== 'I' || word[0][i]== 'L' || word[0][i]== 'N' || word[0][i]== 'O' || word[0][i]== 'R' || word[0][i]== 'S' || word[0][i]== 'T' || word[0][i]== 'U' || word[0][i]== 'a' || word[0][i]== 'e' || word[0][i]== 'i' || word[0][i]== 'l' || word[0][i]== 'n' || word[0][i]== 'o' || word[0][i]== 'r' || word[0][i]== 's' || word[0][i]== 't' || word[0][i]== 'u'){
                                score=score + 1;
                                break;
                            }
            ...
            if(orientation== 'V'){ //o mesmo vai ser replicado para as jogadas na vertical
                    if(board[input_line+i][input_column]=='$'){
                        point_doubler=point_doubler*2;
                        }
                        if(board[input_line+i][input_column]=='3'){
                            if(word[0][i]== 'A' || word[0][i]== 'E' || word[0][i]== 'I' || word[0][i]== 'L' || word[0][i]== 'N' || word[0][i]== 'O' || word[0][i]== 'R' || word[0][i]== 'S' || word[0][i]== 'T' || word[0][i]== 'U' || word[0][i]== 'a' || word[0][i]== 'e' || word[0][i]== 'i' || word[0][i]== 'l' || word[0][i]== 'n' || word[0][i]== 'o' || word[0][i]== 'r' || word[0][i]== 's' || word[0][i]== 't' || word[0][i]== 'u'){
                                score=score + (3*1);
                                break;
                            }
                        ...
                        if(board[input_line+i][input_column]=='2'){
                            if(word[0][i]== 'A' || word[0][i]== 'E' || word[0][i]== 'I' || word[0][i]== 'L' || word[0][i]== 'N' || word[0][i]== 'O' || word[0][i]== 'R' || word[0][i]== 'S' || word[0][i]== 'T' || word[0][i]== 'U' || word[0][i]== 'a' || word[0][i]== 'e' || word[0][i]== 'i' || word[0][i]== 'l' || word[0][i]== 'n' || word[0][i]== 'o' || word[0][i]== 'r' || word[0][i]== 's' || word[0][i]== 't' || word[0][i]== 'u'){
                                score=score + 2*1;
                                break;
                            }
                        ...
                        if(board[input_line+i][input_column]=='.' || board[input_line+i][input_column]=='$' || board[input_line+i][input_column]== word[0][i]){
                            if(word[0][i]== 'A' || word[0][i]== 'E' || word[0][i]== 'I' || word[0][i]== 'L' || word[0][i]== 'N' || word[0][i]== 'O' || word[0][i]== 'R' || word[0][i]== 'S' || word[0][i]== 'T' || word[0][i]== 'U' || word[0][i]== 'a' || word[0][i]== 'e' || word[0][i]== 'i' || word[0][i]== 'l' || word[0][i]== 'n' || word[0][i]== 'o' || word[0][i]== 'r' || word[0][i]== 's' || word[0][i]== 't' || word[0][i]== 'u'){
                                score=score + 1;
                                break;
                            }
